...
<main class="flex-1 min-w-0 w-full overflow-y-auto p-4 sm:p-6 md:p-8">
  <section class="mx-auto w-full max-w-[1600px] space-y-6" aria-labelledby="reliefTitle">
    ...
    <section class="space-y-4" aria-labelledby="beneficiaryTitle">
      <div><h2 id="beneficiaryTitle" class="text-sm font-black text-slate-800">Beneficiary Monitoring</h2><p id="beneficiaryStatus" class="mt-1 text-xs font-bold text-slate-500" role="status" aria-live="polite">Monitoring registered households and assistance.</p></div>
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <?php foreach (['registered_households' => 'Registered Households', 'served_households' => 'Served Households', 'not_yet_served' => 'Not Yet Served', 'people_represented' => 'People Represented'] as $key => $label): ?>
          <article class="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs"><p class="text-[9px] font-black uppercase tracking-wider text-slate-400"><?php echo $label; ?></p><p id="beneficiary-summary-<?php echo $key; ?>" class="mt-2 text-2xl font-black text-slate-800">0</p></article>
        <?php endforeach; ?>
      </div>
      <?php if ($canCreate): ?>
        <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs"><h3 class="text-sm font-black text-slate-800">Register Beneficiary Household</h3><form id="beneficiaryForm" class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2"><input name="household_head_name" maxlength="180" required placeholder="Household head name" class="rounded-lg border border-slate-200 px-3 py-2 text-xs"><select id="beneficiaryBarangay" name="barangay_id" required class="rounded-lg border border-slate-200 px-3 py-2 text-xs"><option value="">Select barangay</option></select><label class="text-[10px] font-bold text-slate-500">Household Size (Number of People)<input name="household_size" type="number" min="1" max="1000" step="1" required placeholder="Number of household members" class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs"></label><input name="contact_number" maxlength="40" placeholder="Contact number (optional)" class="rounded-lg border border-slate-200 px-3 py-2 text-xs"><input name="location_notes" maxlength="1000" placeholder="Address / location notes (optional)" class="rounded-lg border border-slate-200 px-3 py-2 text-xs"><input name="registration_notes" maxlength="2000" placeholder="Registration notes (optional)" class="rounded-lg border border-slate-200 px-3 py-2 text-xs"><button class="relief-action-button sm:col-span-2 w-full rounded-lg bg-slate-900 px-3 py-2.5 text-[10px] font-black uppercase tracking-wider text-white" type="submit"><i class="fa-solid fa-user-plus" aria-hidden="true"></i> Register beneficiary</button></form></section>
      <?php endif; ?>
      <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs"><h3 class="text-sm font-black text-slate-800">Beneficiary Registry</h3><div class="mt-4 overflow-x-auto"><table class="w-full min-w-[860px] text-left text-[11px]"><thead class="border-b border-slate-100 text-[9px] font-black uppercase tracking-wider text-slate-400"><tr><th class="px-2 py-3">Household Head</th><th class="px-2 py-3">Barangay</th><th class="px-2 py-3">Household Size</th><th class="px-2 py-3">Status</th><th class="px-2 py-3">Last Assistance</th><th class="px-2 py-3">Action</th></tr></thead><tbody id="beneficiaryBody" class="divide-y divide-slate-100"></tbody></table></div></section>
    </section>
