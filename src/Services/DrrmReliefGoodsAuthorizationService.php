...
final class DrrmReliefGoodsAuthorizationException extends RuntimeException {}

final class DrrmReliefGoodsAuthorizationService
{
    public const RESOURCE = 'relief goods distribution tracker';
    ...
    public function allows(string $action): bool
    {
        if (!in_array($action, [self::ACTION_VIEW, self::ACTION_CREATE], true)) {
            throw new InvalidArgumentException('Unknown Module 2 action.');
        }
        return $this->isSuperadmin || in_array($action, $this->resourceActions, true);
    }
    public function requireAction(string $action): void
    {
        if (!$this->allows($action)) throw new DrrmReliefGoodsAuthorizationException('Module 2 permission denied.');
    }
    public function capabilities(): array { return ['canView' => $this->canView(), 'canCreate' => $this->canCreate()]; }
