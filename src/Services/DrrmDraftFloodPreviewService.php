...
/**
 * Read-only projection for the single reviewed DENR-MGB flood draft.
 *
 * The dataset UUID and all workflow filters are fixed server-side so this
 * service cannot be used to enumerate arbitrary draft or inactive records.
 */
final class DrrmDraftFloodPreviewService
{
    public const DATASET_VERSION_ID = '1e9c6b5d-dad3-4a5f-bb4b-4c33ac5c30b8';
    public const EXPECTED_FEATURE_COUNT = 15;
    public const SOURCE_SERVICE_URL = 'https://controlmap.mgb.gov.ph/arcgis/rest/services/GeospatialDataInventory/GDI_Detailed_Flood_Susceptibility/FeatureServer/0';

    /** @var array<string, array{risk_level_id: int, mgb_label: string, display_risk_label: string}> */
    private const CLASSIFICATIONS = [
        'LF' => [
            'risk_level_id' => 1,
            'mgb_label' => 'Low Susceptibility to Flooding',
            'display_risk_label' => 'Low',
        ],
        'MF' => [
            'risk_level_id' => 2,
            'mgb_label' => 'Moderate Susceptibility to Flooding',
            'display_risk_label' => 'Moderate',
        ],
        'HF' => [
            'risk_level_id' => 3,
            'mgb_label' => 'High Susceptibility to Flooding',
            'display_risk_label' => 'High',
        ],
        'VHF' => [
            'risk_level_id' => 4,
            'mgb_label' => 'Very High Susceptibility to Flooding',
    ...
    public function __construct(
        ...
        if (!$localDevelopmentPreviewAllowed) {
            throw new RuntimeException('The draft flood preview is unavailable.');
        }
    }
    ...
    public function featureCollection(): array
    {
        ...
        if (count($versions) !== 1) {
            throw new RuntimeException('The controlled flood draft is unavailable.');
        }

        $sourceId = (string) ($versions[0]['dataset_source_id'] ?? '');
        if (!$this->isUuid($sourceId)) {
            throw new RuntimeException('The controlled flood draft has invalid provenance.');
        }
        ...
        if (count($sources) !== 1) {
            throw new RuntimeException('The controlled flood source is unavailable.');
        }
        ...
        if (count($records) !== self::EXPECTED_FEATURE_COUNT) {
            throw new RuntimeException('The controlled flood draft does not contain the expected feature count.');
        }
        ...
        foreach ($records as $record) {
            if (!is_array($record)) {
                throw new RuntimeException('The flood draft returned an unexpected record structure.');
            }

            $notes = $this->decodeNotes($record['classification_notes'] ?? null);
            $code = strtoupper(trim((string) ($notes['mgb_flood_code'] ?? '')));
            ...
            if ($classification === null || $objectId === false || $objectId < 1
                ...
                || ($notes['mgb_flood_label'] ?? null) !== $classification['mgb_label']
                ...
                throw new RuntimeException('The flood draft contains inconsistent source or classification metadata.');
            }
        ...
        if ($counts !== ['LF' => 5, 'MF' => 3, 'HF' => 4, 'VHF' => 3]
            ...
            throw new RuntimeException('The flood draft classification counts are inconsistent.');
        }
    ...
    private function decodeNotes(mixed $value): array
    {
        if (!is_string($value) || $value === '') {
            throw new RuntimeException('The flood draft is missing classification metadata.');
        }

        try {
            $notes = json_decode($value, true, 32, JSON_THROW_ON_ERROR);
        } catch (JsonException) {
            throw new RuntimeException('The flood draft classification metadata is invalid.');
        }
    ...
    private function normalizeGeometry(mixed $geometry): array
    {
        if (is_string($geometry)) {
            try {
                ...
                throw new RuntimeException('A flood draft geometry is not valid GeoJSON.');
            }
        }
        ...
        if (!is_array($geometry)
            ...
            throw new RuntimeException('A flood draft geometry is not a non-empty MultiPolygon.');
        }
    ...
    private function validatePolygon(mixed $polygon): void
    {
        if (!is_array($polygon) || !array_is_list($polygon) || $polygon === []) {
            throw new RuntimeException('A flood draft geometry contains an empty polygon.');
        }

        foreach ($polygon as $ring) {
            if (!is_array($ring) || !array_is_list($ring) || count($ring) < 4) {
                throw new RuntimeException('A flood draft geometry contains an invalid ring.');
            }
            ...
            foreach ($ring as $position) {
                if (!is_array($position) || !array_is_list($position) || count($position) !== 2
                    ...
                    throw new RuntimeException('A flood draft geometry contains an invalid position.');
                }
                ...
                if (!is_finite($longitude) || !is_finite($latitude)
                    ...
                    throw new RuntimeException('A flood draft geometry contains an out-of-range position.');
                }
            }
            ...
            if ((float) $first[0] !== (float) $last[0] || (float) $first[1] !== (float) $last[1]) {
                throw new RuntimeException('A flood draft geometry contains an open ring.');
            }
        }
    }
