...
if (!is_array($input)
    || array_diff(array_keys($input), ['origin', 'evacuation_center_reference_id']) !== []
    ...
    || !is_string($input['evacuation_center_reference_id'] ?? null)) {
    drrmApiRespond(false, null, 'Invalid route request.', 400);
}
...
try {
    ...
    $result = $routeService->adminPlanningRoute(
        (float) $input['origin']['latitude'],
        (float) $input['origin']['longitude'],
        trim($input['evacuation_center_reference_id'])
    );
